...
def get_run_id(
    ...
    Args:
        ...
        dataset: str = name of the dataset, e.g. 'ucsf', 'ucdavis'
    ...
    """
    id_details = f'trf_lags{lag}_bw{bin_width}'
    if identifier != '':
    ...
    if shuffled:
        full_identifier = dataset_name + '_reset_' + identifier
    else:
        full_identifier = dataset_name + '_' + identifier
...
def check_results_across_all_layers(model_names, identifier, num_sessions, verbose=False):
    ...
    for model_name in model_names:
        model_config = load_dnn_config(model_name=model_name)
        num_layers = len(model_config['layers'])
        for i in range(num_layers):
            not_done = check_results(

...
output_dir = manifest['outpur_dir']
...
ridge_model = RidgeRegression(alpha=float(alpha))
...
for l in range(num_layers):
    ...
    for train, val in kf.split(train_val_list):
        ...
        with open(output_dir + "/" + sub + '_' + alpha +".csv" ,'a') as f1:
                writer=csv.writer(f1)
                row = [time.time() - start, r2t, r2v, r2tt]
                writer.writerow(row)
                f1.close()
    
    with open(output_dir + "/" + sub + '_' + alpha + '_all_layers' +".csv" ,'a') as f2:
        writer=csv.writer(f1)
        row = [sub, alpha, l, r2t/5, r2v/5, r2tt/5]
        writer.writerow(row)
        f1.close()
